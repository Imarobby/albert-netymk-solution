...
static list *newList(void)
{
	...
	if(retList==NULL) {
		// printf("Memory failure in newList: can't allocate memory for list");
		return NULL;
	}
	pHead=(listobj *) albert_malloc(1*sizeof(listobj));
	if(pHead==NULL) {
		albert_free(retList);
		// printf("Memory failure in newList: can't allocate memory for pHead");
		return NULL;
	}
	...
	if(pTail==NULL) {
		albert_free(pHead);
		albert_free(retList);
		// printf("Memory failure in newList: can't allocate memory for pTail");
		return NULL;
	}
	retList->pHead=pHead;
	retList->pTail=pTail;
...
static listobj *newListObj(TCB *TCBObj)
{
	...
	if(tmpListObj == NULL) {
		// printf("Memory failure in newListObj: can't allocate memory for tmpListObj");
		return NULL;
	}
